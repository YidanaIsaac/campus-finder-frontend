...
const ItemDetails = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      {/* Fixed Header */}
      <header className="fixed top-0 left-0 right-0 bg-blue-600 text-white p-4 z-50 shadow-md">
        <div className="flex items-center justify-between relative w-full">
          <button onClick={() => navigate(-1)} className="hover:opacity-80 transition-opacity absolute left-0">
            <ArrowLeft className="w-6 h-6" />
          </button>
          <h1 className="text-xl font-semibold text-center w-full">Item Details</h1>
          <button onClick={() => setIsFavorited(!isFavorited)} className="hover:opacity-80 transition-opacity absolute right-0">
            <Heart
